...
data {
  ...
  vector<lower=0>[sum(p)] prior_scale_for_sd_b;
  
  // hyperparameters for random effects model
  vector<lower=0>[t] lkj_shape;
...
parameters {
  ...
  vector[len_b] z_b;
	cholesky_factor_corr[p[1]] L_b_Corr;	// cholesky factor for the random effects correlation matrix
	vector<lower=0>[sum(p)] sd_b_unscaled;	// sds of the random effects
...
transformed parameters {
  ...
  // parameters for random effects model
	vector<lower=0>[sum(p)] sd_b;	// sds of the random effects
  vector[len_b] b; 
  vector[len_b] b_by_model; 

  sd_b = sd_b_unscaled .* prior_scale_for_sd_b;
  ...
  if (t > 0) {
    ...
    for (i in 1:t) {
      for (j in 1:l[i]) {
        // equivalent to: diag_matrix(sd_b) * L_b_Corr * z_b;
        b[(j_shift+1):(j_shift+p[i])] <- 
          sd_b[(i_shift+1):(i_shift+p[i])] .* (L_b_Corr * z_b[(j_shift+1):(j_shift+p[i])]);
        j_shift = j_shift + p[i];
      }
      i_shift = i_shift + p[i];
    }
...
#    if (y_has_noise[m] == 1)      nois_mark = nois_mark + 1;
  ...
  if (t > 0) {
    target += cauchy_lpdf(sd_b_unscaled | 0, 1);
    for (i in 1:t) 
      target += lkj_corr_cholesky_lpdf(L_b_Corr | lkj_shape[i]);
  } 
